...
const createData = () => {
  if (props.id) {
    axios
      .patch(`bino/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('bino', key)
          // router.push({ path: "/bino" });
        }
      });
  } else {
    axios.post("bino/create", Data.value).then((res) => {
      // console.log(res)
      if (res.data) {
        let key = "bino_id";
        emit("bino", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`bino/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
