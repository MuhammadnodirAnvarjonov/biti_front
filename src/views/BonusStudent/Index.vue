<template>
    <div>
      ...
      <n-modal
        ...
        size="huge"
      >
      <FormVue @bonustalaba="resData" :id="Id" />
      </n-modal>
    <div class="DataTable">
      <!-- <n-data-table
        size="large"
...
// const routerPush = () => {
//     router.push({ path: "/bonus/student/create"})
// };
const routeUpdate = (id) => {
    Id.value = id;
    showModal.value = true;
    // router.push({ path: "/bonus/student/update/" + id})
};
const resData = (key) => {
  get();
...
const deletedata = (id) => {
    dialog.warning({
          ...
          onPositiveClick: () => {
            axios.delete('/bonus_student/delete/' + id).then(res =>{
            if(res){
                get();
            }
        })
