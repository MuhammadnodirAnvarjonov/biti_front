...
const createData = () => {
  if (props.id) {
    axios
      .patch(`programm/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('dastur', key)
          // router.push({ path: "/programm" });
        }
      });
  } else {
    axios.post("programm/create", Data.value).then((res) => {
      if (res.data) {
        let key = "programm";
        emit("dastur", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`programm/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
