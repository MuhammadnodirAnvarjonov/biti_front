<template>
  <div>
    <div class="createheader">
      <span>Guruh rahbari qo'shish</span>
      <!-- <div>
      ...
      <div style="display: flex; justify-content: space-between">
        <div style="width: 49%">
          ...
          <n-input
            ...
            clearable
            size="large"
            placeholder="Guruh rahbarini kiriting"
          />
        </div>
      </div>
...
const emit = defineEmits(["guruhrabar"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`group_leader/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('guruhrabar', key);
          // router.push({ path: "/group/leader" });
        }
      });
  } else {
    axios.post("group_leader/create", Data.value).then((res) => {
      if (res.data) {
        let key = "group_leader_id";
        emit("guruhrabar", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`group_leader/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
