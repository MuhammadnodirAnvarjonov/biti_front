  ...
  <div>
    <n-card class="cardTable">
      ...
      <n-modal
        ...
        size="huge"
      >
      <FormVue @guruhrabar="resData" :id="Id" />
      </n-modal>
    <div class="DataTable">
      <!-- <n-data-table
        size="large"
...
// const routerPush = () => {
//   router.push({ path: "/group/leader/create" });
// };
const routeUpdate = (id) => {
  Id.value = id;
  showModal.value = true;
  // router.push({ path: "/group/leader/update/" + id });
};
const resData = (key) => {
  get();
...
const get = () => {
  axios.get("/group_leader/all").then((res) => {
    if (res) {
      Data.value = res.data;
    }
  });
...
const deletedata = (id) => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      axios.delete("/group_leader/delete/" + id).then((res) => {
        if (res) {
          get();
        }
      });
