  ...
  <div>
    <n-card class="cardTable">
      ...
      <n-modal
          ...
          size="huge"
        >
        <FormVue @ilmiyrahbar="resData" :id="Id" />
        </n-modal>
      <div class="DataTable">
        <!-- <n-data-table
          size="large"
...
// const routerPush = () => {
//   router.push({ path: "/ilmiy/rahbar/create" });
// };
const routeUpdate = (id) => {
  Id.value = id;
  showModal.value = true;
  // router.push({ path: "/ilmiy/rahbar/update/" + id });
};
const get = () => {
  axios.get("/scientific_leader/all").then((res) => {
    if (res) {
      Data.value = res.data;
    }
  });
...
const deletedata = (id) => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      axios.delete("/scientific_leader/delete/" + id).then((res) => {
        if (res) {
          get();
        }
      });
