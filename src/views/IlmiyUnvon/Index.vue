  ...
  <div>
    <n-card class="cardTable">
      ...
      <n-modal
        ...
        size="huge"
      >
      <FormVue @ilmiyunvon="resData" :id="Id" />
      </n-modal>
    <div class="DataTable">
      <!-- <n-data-table
        size="large"
...
// const routerPush = () => {
//   router.push({ path: "/academic/title/create" });
// };
const routeUpdate = (id) => {
  Id.value = id;
  showModal.value = true;
  // router.push({ path: "/academic/title/update/" + id });
};
const resData = (key) => {
  get();
...
const deletedata = (id) => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      axios.delete("/academic_title/delete/" + id).then((res) => {
        if (res) {
          get();
        }
      });
