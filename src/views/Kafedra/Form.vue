...
const emit = defineEmits(["kafedra"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios.patch(`kafedra/update/` + props.id, Data.value).then((res) => {
      if (res.data) {
        let key = false;
        emit('kafedra', key)
        // router.push({ path: "/kafedra" });
      }
    });
  } else {
    axios.post("kafedra/create", Data.value).then((res) => {
      if (res.data) {
        let key = "kafedra_id";
        emit("kafedra", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`kafedra/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
