...
const emit = defineEmits(["lavozimi"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`position/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('lavozimi', key);
    ...
    axios.post("position/create", Data.value).then((res) => {
      if (res.data) {
        let key = "position_id";
        emit("lavozimi", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`position/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
