<template>
  <div>
    <div class="createheader">
      <span>Ma'lumotini qo'shish</span>
      <!-- <div>
      ...
      <div style="display: flex; justify-content: space-between">
        <div style="width: 49%">
          ...
          <n-input
            ...
            clearable
            size="large"
            placeholder="Ma'lumotini kiriting"
          />
        </div>
      </div>
...
const emit = defineEmits(["malumoti"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`information/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('malumoti', key);
    ...
    axios.post("information/create", Data.value).then((res) => {
      if (res.data) {
        let key = "information_id";
        emit("malumoti", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`information/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
