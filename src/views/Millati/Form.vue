...
const emit = defineEmits(["millati"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`national/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('millati', key)
          // router.push({ path: "/national" });
        }
      });
  } else {
    axios.post("national/create", Data.value).then((res) => {
      if (res.data) {
        let key = "nationality_id";
        emit("millati", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`national/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
