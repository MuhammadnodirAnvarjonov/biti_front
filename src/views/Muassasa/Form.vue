...
const emit = defineEmits(["muassasa"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`institution/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('muassasa', key);
          // router.push({ path: "/institution" });
        }
      });
  } else {
    axios.post("institution/create", Data.value).then((res) => {
      if (res.data) {
        let key = "institution_id";
        emit("muassasa", key, res.data);
      }
    });
  }
};
...
const getOne = () => {
  if (props.id) {
    axios.get(`institution/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
