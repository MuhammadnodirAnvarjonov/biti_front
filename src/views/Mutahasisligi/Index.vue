  ...
  <div>
    <n-card class="cardTable">
      ...
      <n-modal
        ...
        size="huge"
      >
      <FormVue @mutahasiligi="resData" :id="Id" />
      </n-modal>
    <div class="DataTable">
      <!-- <n-data-table
        size="large"
        ...
        >
          <thead>
            <tr>
              ...
              <th>Ma'lumoti bo'yicha mutahasisligi</th>
...
// const routerPush = () => {
//   router.push({ path: "/special/information/create" });
// };
const routeUpdate = (id) => {
  Id.value = id;
  showModal.value = true;
  // router.push({ path: "/special/information/update/" + id });
};
const resData = (key) => {
  get();
...
const deletedata = (id) => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      axios.delete("/special_information/delete/" + id).then((res) => {
        if (res) {
          get();
        }
      });
