  ...
  <div>
    <n-card class="cardTable">
      ...
      <n-modal
        ...
        size="huge"
      >
      <FormVue @oilaviyahvoli="resData" :id="Id" />
      </n-modal>
    <div class="DataTable">
      <!-- <n-data-table
        size="large"
...
// const routerPush = () => {
//   router.push({ path: "/family/status/create" });
// };
const routeUpdate = (id) => {
  Id.value = id;
  showModal.value = true;
  // router.push({ path: "/family/status/update/" + id });
};
const resData = (key) => {
  get();
...
const deletedata = (id) => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      axios.delete("/family_status/delete/" + id).then((res) => {
        if (res) {
          get();
        }
      });
