...
const createData = () => {
  if (props.id) {
    axios
      .patch(`language/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('til', key);
    ...
    axios.post("language/create", Data.value).then((res) => {
      if (res.data) {
        Data.value = {
          name: "",
        };
        let key = "language_skills";
        emit("til", key, res.data);
      }
...
const getOne = () => {
  if (props.id) {
    axios.get(`language/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
