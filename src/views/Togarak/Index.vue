...
// const routerPush = () => {
//     // router.push({ path: "/togarak/create"})
// };
const routeUpdate = (id) => {
    // router.push({ path: "/togarak/update/" + id})
    Id.value = id;
...
const deletedata = (id) => {
    dialog.warning({
          ...
          onPositiveClick: () => {
            axios.delete('/togaraklar/delete/' + id).then(res =>{
            if(res){
                get();
            }
        })
...
<template>
    ...
    <n-card class="cardTable">
        <div class="cardheader">
                <div>
                    <n-button class="add-btn" type="error" size="large" @click="showModal = true, Id = null">
                        <n-icon size="30">
                        <AddCircleSharp />
                        </n-icon>
                        Qo'shish
            ...
            <n-modal
                ...
                size="huge"
            >
            <FormVue @togaraklar="resData" :id="Id" />
            </n-modal>
            <div class="DataTable">
                <n-space vertical>
                    <n-table striped>
