...
const emit = defineEmits(["yonalish"]);
const Data = ref({
  name: "",
});
const createData = () => {
  if (props.id) {
    axios
      .patch(`direction/update/` + props.id, Data.value)
      .then((res) => {
        if (res.data) {
          let key = false;
          emit('yonalish', key);
    ...
    axios.post("direction/create", Data.value).then((res) => {
      if (res.data) {
        Data.value = {
          name: "",
        };
        let key = "direction_id";
        emit("yonalish", key, res.data);
      }
...
const getOne = () => {
  if (props.id) {
    axios.get(`direction/one/` + props.id).then((res) => {
      if (res) {
        Data.value = res.data;
      }
    });
